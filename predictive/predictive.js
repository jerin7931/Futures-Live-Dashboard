(() => {
  ...
  function patch(table,row){const started=performance.now();ingest(table,row);if(table==="predictive_model_state_live"||table==="predictive_signal_episode_live")renderModel(row.model_id||row.payload?.model_id);else if(table==="predictive_market_context_live")renderContexts();else if(table==="predictive_gex_surface_live")renderGex();else if(table==="predictive_option_ladder_live")renderLadder();else renderHealth();window.dispatchEvent(new CustomEvent("predictive-render-latency",{detail:{table,ms:performance.now()-started,received_at:new Date().toISOString()}}));}
