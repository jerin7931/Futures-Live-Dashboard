(() => {
  ...
  function updateCardHealth(id){const row=state.models[id],card=state.cards.get(id);if(!card)return;const modelAge=currentAge(row?.model_event_time,row?.model_age_ms),quoteAge=currentAge(row?.latest_quote_time,row?.quote_age_ms),guidance=row?.guidance_state||"BLOCKED",modelFresh=Boolean(row)&&modelAge!=null&&modelAge<=(cfg.stalenessMs?.quantOptionEvent||90000);$(".model-age",card).textContent=`Model ${age(modelAge)}`;$(".quote-age",card).textContent=`Quote ${age(quoteAge)}`;$(".model-health-label",card).textContent=`MODEL ${modelFresh?"LIVE":"STALE"}`;$(".data-label",card).textContent=`DATA ${guidance}`;$(".state-chip",card).textContent=guidance;card.classList.toggle("runtime-stale",guidance==="STALE"||!modelFresh||quoteAge>(cfg.stalenessMs?.webullQuote||5000));}
