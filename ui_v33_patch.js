...
function tvKind(l){const x=String(l.type||"").toUpperCase();return x.includes("RESIST")?"R":x.includes("SUPPORT")?"S":"P";}
...
function buildTvExecutionPacket(){const es=latestQuote("ES"),mes=latestQuote("MES"),esPx=Number(es?.last),mesPx=Number(mes?.last);if(!Number.isFinite(esPx)||!Number.isFinite(mesPx))return null;const modelIds=new Set((state.models||[]).map(x=>x.level_id).filter(Boolean)),important=(state.levels||[]).filter(r=>r.active===true&&tvPenalty(r)<99&&Math.abs(Number(r.reference_price)-esPx)<=30).map(tvRange).filter(Boolean);const records=[],zones=[];let modelCount=0,ofCount=0,nyCount=0;const std=(state.models||[]).find(x=>x.role==="STANDARD"),watch=(state.models||[]).find(x=>x.role==="WATCH"),model=std||watch;const consumed=new Set();if(model&&tvServingLive()){const row=(state.levels||[]).find(x=>x.level_id===model.level_id),base=tvRange(row||model);if(base){const near=important.filter(x=>x.r.level_id!==model.level_id&&x.lo<=base.hi+.75&&x.hi>=base.lo-.75&&Math.max(base.hi,x.hi)-Math.min(base.lo,x.lo)<=4);near.forEach(x=>consumed.add(x.r.level_id));const lo=Math.min(base.lo,...near.map(x=>x.lo)),hi=Math.max(base.hi,...near.map(x=>x.hi)),extra=tvClusterLabel(near),label=[tvCode(model.level_type),near.length?extra:null].filter(Boolean).join("+");const rej=Number.isFinite(Number(model.rejection_probability))?Math.round(Number(model.rejection_probability)*100):-1,role=std?"STD":"WATCH";records.push(`M,${tvNum(lo)},${tvNum(hi)},${label},${rej},${role}`);zones.push({lo,hi,major:true});modelCount=1;}}
const rest=important.filter(x=>!modelIds.has(x.r.level_id)&&!consumed.has(x.r.level_id));for(const c of tvClusters(rest,mesPx).slice(0,4)){records.push(`O,${tvNum(c.lo)},${tvNum(c.hi)},${tvClusterLabel(c.items)}`);zones.push({lo:c.lo,hi:c.hi,major:false});ofCount++;}
const nyAll=roadLevels().filter(l=>!["REMOVED","DOWNGRADED"].includes(String(l.status||"").toUpperCase())).map(l=>({...l,lo:tvTick(Math.min(l.ml,l.mh)),hi:tvTick(Math.max(l.ml,l.mh)),mid:tvTick((l.ml+l.mh)/2),major:["MAJOR","STRUCTURAL","PSYCHOLOGICAL"].includes(String(l.importance||"").toUpperCase())}));const nearNy=nyAll.filter(l=>Math.abs(l.mid-mesPx)<=35).sort((a,b)=>Math.abs(a.mid-mesPx)-Math.abs(b.mid-mesPx)),chosen=[];for(const l of nearNy){const dup=zones.some(z=>l.lo<=z.hi+.25&&l.hi>=z.lo-.25);if(dup&&!l.major)continue;if(!chosen.includes(l))chosen.push(l);if(chosen.length>=3)break;}const major=nyAll.filter(l=>l.major&&!chosen.includes(l)).sort((a,b)=>Math.abs(a.mid-mesPx)-Math.abs(b.mid-mesPx))[0];if(major&&chosen.length<4)chosen.push(major);for(const l of chosen.slice(0,4)){records.push(`N,${tvNum(l.lo)},${tvNum(l.hi)},${tvKind(l)},${tvStatusCode(l.status)},${tvNyCode(l)}`);nyCount++;}
if(!records.length)return null;const body=records.join("|"),version=tvHash(body),packet=`H,${version}|${body}`;return{packet,version,modelCount,ofCount,nyCount,total:records.length,gate:tvServingLive()};}
...
function renderTvSync(){const host=$("tvLevelSyncPatch");if(!host)return;const x=buildTvExecutionPacket();window.FM_V33_TV_EXECUTION_PACKET=x?.packet||null;window.FM_V33_TV_EXECUTION_VERSION=x?.version||null;const status=$("tvSyncStatusPatch"),meta=$("tvSyncMetaPatch"),copy=$("tvCopyLevelsPatch"),mark=$("tvMarkSyncedPatch");if(!x){status.textContent="Waiting for execution levels";status.className="tv-sync-status changed";meta.textContent="Need fresh ES/MES quotes and loaded level state.";copy.disabled=true;mark.classList.add("hidden");return;}copy.disabled=false;let synced=null,copied=null;try{synced=localStorage.getItem(TV_SYNC_KEY);copied=localStorage.getItem(TV_COPY_KEY);}catch{}if(synced===x.version){status.textContent=`TradingView synced — ${x.version}`;status.className="tv-sync-status synced";copy.textContent="COPY AGAIN";mark.classList.add("hidden");}else if(copied===x.version){status.textContent=`Copied ${x.version} — paste into TradingView`;status.className="tv-sync-status copied";copy.textContent="COPY AGAIN";mark.classList.remove("hidden");}else{status.textContent=`Levels changed — ${x.version}`;status.className="tv-sync-status changed";copy.textContent="COPY NEW LEVELS";mark.classList.add("hidden");}meta.textContent=`${x.total} plotted entities · ${x.modelCount?"model + ":""}${x.ofCount} OF confluence · ${x.nyCount} NY map · ${x.gate?"Order Flow LIVE":"Order Flow model parked; structural levels only"}`;copy.dataset.packet=x.packet;mark.dataset.version=x.version;}
...
function liveStructural(row){if(row.symbol!=="ES"||!state.candles)return;setLiveFromQuote(row);if(chartMode("structural")==="linebreak"){const rr=window.FM_V33_RENDER_STRUCTURAL_CHART;if(typeof rr==="function")rr();return;}const b=P.live[key("ES",state.tf)];if(b)try{state.candles.update(chartBar(b));}catch{}}
