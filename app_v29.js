...
function chartSignalAtTime(time){
  ...
  if(!Number.isFinite(clickTime))return null;
  const bars=completedBars(state.symbol,state.tf);
  const byClose=new Map(bars.map(b=>[Number(b.closeMs),Number(b.time)]));
  return state.events
    .filter(e=>e.root===state.symbol&&e.timeframe===state.tf)
    .map(e=>({event:e,markerTime:byClose.get(new Date(e.signal_close_utc).getTime())}))
    .filter(x=>Number.isFinite(x.markerTime)&&x.markerTime===clickTime)
    .sort((a,b)=>new Date(b.event.signal_close_utc)-new Date(a.event.signal_close_utc))[0]?.event||null;
}
function selectModelSignal(e){
  if(!e)return;
  state.selectedEvent=e.event_id;
  state.symbol=e.root;
  state.tf=e.timeframe;
  $$('[data-symbol]').forEach(x=>x.classList.toggle('active',x.dataset.symbol===state.symbol));
  $$('[data-tf]').forEach(x=>x.classList.toggle('active',x.dataset.tf===state.tf));
  renderDecisions();
  renderQueue();
  renderContext();
  renderChart(false);
...
function installChartSignalClick(){
  ...
  state.chart._modelSignalClickInstalled=true;
  state.chart.subscribeClick(param=>{
    const e=chartSignalAtTime(param?.time);
    if(e)selectModelSignal(e);
  });
}
