...
export function buildDemoData(now=Date.now()){
  ...
  const market=[
    ["SPY","SPY",692.41,.54],["QQQ","QQQ",624.87,.88],["IWM","IWM",247.62,-.31],["VIX","Volatility",17.34,-2.12],["US10Y","10Y Treasury",4.16,.03],["US2Y","2Y Treasury",3.72,.02],["DXY","U.S. Dollar",97.63,-.18],["WTI","WTI Crude",67.44,1.21],
  ].map(([symbol,label,value,change],index)=>({symbol,label,value,change_1d_pct:change,move_1m_pct:round(change*.03),move_3m_pct:round(change*.07),move_5m_pct:round(change*.12),move_10m_pct:round(change*.18),move_15m_pct:round(change*.24),v1_state:index<3?STATES[index+1]:null,efficiency:index<3?.5+index*.08:null,timestamp:iso(now-9000),source:"SIMULATED_PROVIDER",freshness:{state:"LIVE",age_seconds:9}}));
