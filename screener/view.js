...
export function trendCardsHTML(payload,now){
  ...
  return '<div class="section-title"><h2>Synthetic Trend Radar</h2><span>ISOLATED · NOT A SIGNAL</span></div><p class="section-note">Scenario '+esc(trend.scenarioId)+' · deterministic simulated clock · no production activation.</p><div class="cards">'+trend.states.map(item=>{
    ...
    const context=item.research_context??{},tm=context.trend_magic??{},market=context.market??{},sector=context.sector??{},catalyst=context.catalyst??{};
    const tmText=[5,15,30,60].map(frame=>frame+'m '+esc(tm[String(frame)]??"—")).join(' · ');
    const contextLine=context.descriptive_only===true?'<small class="trend-context">TM '+tmText+'<br>MARKET '+esc(market.label??"—")+' · SECTOR '+esc(sector.label??"—")+' · CATALYST '+esc(catalyst.label??"—")+'<br>RANK CONTEXT '+esc(context.rank_effect??"DESCRIPTIVE ONLY")+'</small>':'';
    return '<article class="panel setup trend-synthetic"><div class="card-title"><h2>'+esc(item.symbol)+'</h2>'+badge(item.status,item.status==="DEGRADING"||item.status==="REVERSED"||item.status==="EXPIRED")+'</div><p>'+badge(item.direction)+' Detector '+esc(item.detector_state??item.state)+'</p><dl>'+pair("Entry trigger",esc(trigger?.family??"—")+" · "+money(trigger?.price))+pair("Current price",money(item.price))+pair("Move / efficiency",signed(move)+"% / "+number(item.efficiency,2))+pair("Rank",Number.isFinite(item.rank)?"#"+esc(item.rank):"—")+'</dl>'+contextLine+'<small>WHY NOW · '+esc(why)+'</small><small>Trigger age '+age(trigger?.at)+' · state age '+age(item.lifecycle?.observed_at)+' · source '+esc(item.source_valid===true?"FRESH":"NOT CURRENT")+'</small><small>Evidence '+esc(clock(item.basis_end_at))+' · expires '+esc(clock(item.lifecycle?.valid_until))+'</small></article>';
  }).join("")+'</div>';
}
